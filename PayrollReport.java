			...
			while(s.hasNext()) {
				...
				if(!line.trim().isEmpty()) {
					...
					if(tokens[0].equals("E")) {
						...
						e = new Temporary(id,lastName, firstName, title,hourlyPayRate,hoursWorked);
					}
	...
	public static void main(String args[]) {
		...
		for(Employee e : payroll) {
			...
			sb.append(String.format("%-8s %-20s %-10s %-30s $%8.2f $%8.2f $%8.2f\n", 
					e.getId(), e.getLastName(), e.getFirstName(), ((SalaryEmployee) e).getType(), e.getGrossPay(), e.getTaxes(), e.getNetPay())); //TODO: replace these
		}
